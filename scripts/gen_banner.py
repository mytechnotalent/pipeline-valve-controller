#!/usr/bin/env python3
"""Generate the pipeline-valve-controller.png banner (1500x1500, pure stdlib).
...
def _draw_titles(banner, colors) -> None:
    ...
    None
    """
    banner.text("SCADA PIPELINE VALVE CONTROL", 4,
                colors["gold"], banner.W // 2, 90)
    banner.text("OPERATION IRON VEIN", 10, (0x0B, 0x10, 0x26),
                banner.W // 2 - 6, 176)
    banner.text("OPERATION IRON VEIN", 10, colors["white"],
                banner.W // 2, 182)
    banner.text("RP2350  PICO 2", 5, colors["cyan"], banner.W // 2, 560)
    banner.text("-=[ SCADA DEFENSE + MALWARE ANALYSIS LAB ]=-", 3, colors["dim"],
                banner.W // 2, 660)
...
def _draw_features(banner, colors) -> None:
    ...
    None
    """
    banner.text("PIPELINE_VALVE_CONTROLLER", 5, colors["cyan"],
                banner.W // 2, 750)
    banner.text("VALVE ACTUATOR  ESTOP BUTTON", 4, colors["silver"],
                banner.W // 2, 900)
    banner.text("IR OPERATOR REMOTE  DHT11 PROCESS", 3, colors["silver"],
                banner.W // 2, 990)
    banner.text("XCHACHA20-POLY1305  ARGON2ID", 4, colors["dim"],
                banner.W // 2, 1120)
    banner.text("IMPLANT ANALYSIS  ANTI-DEBUG", 4, colors["gold"],
                banner.W // 2, 1260)
    banner.text("FAIL-CLOSED  AUTHENTICATED STATE", 4, colors["gold"],
                banner.W // 2, 1350)
    banner.text("-=[ OPERATION IRON VEIN  /  RP2350 ]=-", 3, colors["dim"],
                banner.W // 2, 1440)
...
def main() -> int:
    ...
    out = Path(__file__).resolve().parent.parent / "pipeline-valve-controller.png"
    banner.save(out)

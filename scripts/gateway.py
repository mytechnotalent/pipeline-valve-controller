#!/usr/bin/env python3
"""SCADA gateway that authorizes and answers sealed valve requests.
...
authenticated with the valve node identifier byte 0x07. An authenticated
request is decrypted to recover the requested valve command, logged, and
answered with a sealed valve command carrying a monotonic sequence number
and an authenticated state tag over the resulting authorization record.
The tag is computed with the field key, so a forged command cannot be
produced without it, and the sequence window means a captured command
cannot be replayed.

The CSV log has the columns utc, sender, auth, cmd, rssi_snr. The auth
column is OK for an authenticated request and UNAUTHENTICATED for a
rejected frame. The cmd column holds the recovered command for an
authenticated request and an empty string for a rejected frame, so no
forged command ever enters the log as truth.
"""
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the VALVE gateway"
    ) from exc

CSV_HEADER = ("utc", "sender", "auth", "cmd", "rssi_snr")
HUB_ADDRESS = 1
RADIO_NETWORK_ID = 18
...
def _parse_args():
    ...
    parser.add_argument("--log", default="valve_log.csv",
...
def _field_key():
    ...
    -------
    bytes
        Thirty-two byte field key shared with the valve firmware.
    """
    global _FIELD_KEY
...
def _command_body(seq, cmd):
    """Build the signed body of a sealed valve command.
    ...
    seq : int
        Sequence number carried by the command.
    cmd : int
        Open or close command byte.

    Returns
    -------
    bytes
        Twenty-one byte command body of sequence, command, and tag.
    """
    return _le32(seq) + bytes([cmd]) + _state_tag(seq)


def _seal_command(seq, cmd):
    """Seal a valve command with the field key.
    ...
    """
    return field_crypto.seal_field_frame(_command_body(seq, cmd))
...
def _grant(ser, sender, cmd):
    """Answer an authenticated request with a sealed valve command.
    ...
    """
    seq = next(_SEQ)
    print(f"VALVE seq={seq} cmd={cmd}", flush=True)
    _send_at(ser, sender, _seal_command(seq, cmd))


def _write_row(writer, handle, auth, record, cmd):
    ...
    cmd : str
        Recovered command text, or an empty string when rejected.
    ...
    writer.writerow((stamp, sender, auth, cmd, tail))
    handle.flush()


def _open_request(record):
    """Authenticate a request and recover the guarded valve command.
    ...
    -------
    int or None
        Recovered command byte, or None when authentication fails.
    """
    try:
        plaintext = field_crypto.open_field_frame(record[1])
    except (ValueError, UnicodeDecodeError):
        return None
    if len(plaintext) != 1 or plaintext[0] > VALVE_COMMAND_OPEN:
        return None
    return plaintext[0]


def _accept(ser, writer, handle, record, cmd):
    ...
    None
    """
    _write_row(writer, handle, "OK", record, str(cmd))
    _grant(ser, record[0], cmd)
...
def _handle_line(ser, writer, handle, line):
    ...
    if record is None:
        return
    cmd = _open_request(record)
    if cmd is None:
        _reject(writer, handle, record)
        return
    _accept(ser, writer, handle, record, cmd)

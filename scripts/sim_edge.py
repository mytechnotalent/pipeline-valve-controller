#!/usr/bin/env python3
"""Simulate a real pipeline valve controller from a laptop without a Pico.

sim_edge.py drives a USB-to-TTL radio with the same sealed request and
AT+SEND cadence as the RP2350 firmware. Each requested valve command is
sealed with the OPERATION COLD IRON field key so the SCADA gateway
accepts the request and answers with a sealed valve command. Students can
validate the gateway and then practice spoofing against a live controller
before touching embedded hardware.
"""
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the VALVE edge simulator"
    ) from exc
...
def _tick(ser, args, seq):
    """Transmit one simulated sealed valve request.
    ...
    None
    """
    envelope = field_crypto.seal_field_frame(bytes([COMMANDS[args.cmd]]))
    cmd = f"AT+SEND={HUB_ADDRESS},{len(envelope)},{envelope}\r\n"
    ser.write(cmd.encode("utf-8"))
    ser.flush()
    print(f"[node {args.node} seq {seq}] sealed {args.cmd}", flush=True)

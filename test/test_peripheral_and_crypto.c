 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 IRON VEIN pipeline valve controller
 * status LED, ESTOP button, valve servo, infrared decoder, the ChaCha20,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(VALVE_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(VALVE_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(valve_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(VALVE_OFF, 0, 0, 0);
    assert_show(VALVE_FAULT, 1, 0, 0);
    assert_show(VALVE_PENDING, 0, 1, 0);
    assert_show(VALVE_NOMINAL, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_estop_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(estop_init());
    gpio_put(VALVE_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(estop_pressed());
    gpio_put(VALVE_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(estop_pressed());
}
 ...
 * @return void
 */
static void test_estop_consume(void) {
    reset_pc();
    estop_init();
    gpio_put(VALVE_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(estop_consume_press());
    gpio_put(VALVE_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(estop_consume_press());
    TEST_ASSERT_FALSE(estop_consume_press());
}
 ...
 * @return void
 */
static void estop_release(void) {
    gpio_put(VALVE_BUTTON_PIN, true);
    estop_consume_press();
}
 ...
 * @return void
 */
static void test_estop_debounce(void) {
    reset_pc();
    estop_init();
    gpio_put(VALVE_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(estop_consume_press());
 ...
 * @return void
 */
static void test_estop_debounce_elapsed(void) {
    reset_pc();
    estop_init();
    gpio_put(VALVE_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(estop_consume_press());
    estop_release();
    mock_timer_set_us(mock_timer_now_us() + ESTOP_DEBOUNCE_US);
    gpio_put(VALVE_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(estop_consume_press());
}

/**
 * @brief Verify estop_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_estop_reset(void) {
    reset_pc();
    estop_init();
    gpio_put(VALVE_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(estop_consume_press());
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(VALVE_SERVO_PIN);
...
static void test_servo_actuate(void) {
    ...
    valve_open();
    assert_servo_pulse(servo_angle_to_pulse_us(SERVO_ANGLE_OPEN_DEGREES));
    valve_close();
    assert_servo_pulse(servo_angle_to_pulse_us(SERVO_ANGLE_CLOSED_DEGREES));
}
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[VALVE_IR_PIN]);
}
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_estop_pressed);
    RUN_TEST(test_estop_consume);
    RUN_TEST(test_estop_debounce);
    RUN_TEST(test_estop_debounce_elapsed);
    RUN_TEST(test_estop_reset);
}

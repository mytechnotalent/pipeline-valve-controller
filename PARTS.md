# OPERATION IRON VEIN - Hardware Parts and Products
...
The hardware below builds the OPERATION IRON VEIN valve controller and its
classroom labs. It is the same breadboard as Act I and Act II, so every
peripheral in the Embedded Hacking kit is **required**: the servo is the valve
actuator, the button is the emergency stop (ESTOP), the infrared eye is the
operator remote (OPEN, CLOSE, and ESTOP), the LEDs annunciate VALVE FAULT,
COMMAND PENDING, and VALVE NOMINAL, the DHT11 is the process sensor, the 1602 LCD
is the SCADA status readout, and the RYLR998 radio carries the sealed valve
command and audit uplink that the labs protect.

The malware track needs one more thing: a **Debug Probe**. It is how you watch
the covert beacon, trigger the logic bomb, and step past the anti-debug trap, so
...
| ---- | ------------- | ----- |
| Legitimate sealed valve command loop | **2** | 1x valve-node RYLR998 (on the Pico) + 1x gateway RYLR998 (USB-to-TTL) |
...
- [1x DHT11 temperature and humidity sensor (process sensor)](https://www.amazon.com/s?k=dht11+temperature+and+humidity+sensor)
...
- [1x Push button (tactile switch, the emergency stop)](https://www.amazon.com/s?k=tactile+push+button+assortment)

### Actuator and infrared control surface

- [1x SG90 servo motor (the valve actuator)](https://www.amazon.com/s?k=sg90+micro+servo+motor)
...
- [1x Infrared (IR) remote controller (NEC-compatible operator remote)](https://www.amazon.com/s?k=arduino+ir+remote+control)

### LoRa radios and serial adapters

- [3x RYLR998 LoRa module with antenna (valve node, gateway, and attacker)](https://www.amazon.com/s?k=rylr998+lora+module)
...
| ---------- | ---- | ----- |
| DHT11 process sensor | GP4 | 10K pull-up required |
| 1602 LCD SDA | GP2 | I2C1 |
| 1602 LCD SCL | GP3 | I2C1, address 0x27 |
| RYLR998 TX (Pico RX) | GP9 | UART1 |
| RYLR998 RX (Pico TX) | GP8 | UART1 |
| Infrared operator remote | GP5 | VS1838B, active low, OPEN/CLOSE/ESTOP |
| Valve servo signal | GP14 | PWM, 50 Hz, 1000uF bulk on the 5V rail |
| Red VALVE FAULT LED | GP16 | 220-330 Ohm to ground |
| Yellow COMMAND PENDING LED | GP17 | 220-330 Ohm to ground |
| Green VALVE NOMINAL LED | GP18 | 220-330 Ohm to ground |
| Emergency stop button | GP15 | Active low, internal pull-up, highest priority |
...
- **Argon2id** (RFC 9106) derives the 256-bit field key from a provisioned
  ...
  fits the RP2350 SRAM budget; raise it on the SCADA gateway.
- **XChaCha20-Poly1305** seals every request and command with a 192-bit nonce and
  ...
  never moves the valve.

Act III adds stateful security on top of the sealed wire:
...
- **Authenticated state tag.** A keyed tag over the authorization record means a
  debugger that flips the SRAM `granted` boolean is rejected before the valve
  moves.
...
The FROSTLINE implant is compiled only under `SANDBOX_ONLY`. It beacons every 8
ticks with the magic `DE AD BE EF`, arms on the 8-byte magic `FROSTLNE`, closes
the valve 3 ticks later, checks CoreDebug `DHCSR` at `0xE000EDF0` to hide from a
probe, and erases and programs a one-time `0xC7` marker into the reserved flash
sector `0x103FF000`.
It is real in technique and inert in effect: it touches only its own pins, its own
radio frame, and its reserved sector, on your own breadboard.
